...
mod tests {
    ...
    fn parity_is_14_bytes() {
        let h = Hash::hash(b"14 bytes").expect("hashing should succeed");
        assert_eq!(h.parity().len(), 14);
    }

    #[test]
    fn parity_is_suffix_of_inner() {
        let h = Hash::hash(b"suffix").expect("hashing should succeed");
        let parity = h.parity();
    ...
    fn parity_enables_error_correction() {
        let original = Hash::hash(b"error correction").expect("hashing should succeed");
