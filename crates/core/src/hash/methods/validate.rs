...
mod tests {
    ...
    fn validate_uncorrupted_binary() {
        let original = Hash::hash(b"test").expect("hashing should succeed");
        let validated = Hash::validate(original.inner)
    ...
    fn validate_single_bit_corruption_base64() {
        ...
        let recovered =
            Hash::validate(&corrupted_str).expect("single-bit corruption should be recovered");
    ...
    fn validate_single_byte_corruption_binary() {
        ...
        corrupted[10] ^= 0xFF;
        let recovered =
            Hash::validate(corrupted).expect("single-byte corruption should be recovered");
        assert_eq!(original, recovered);
    }

    #[test]
    fn validate_multi_byte_corruption_recoverable() {
        let original = Hash::hash(b"multi").expect("hashing should succeed");
        let mut corrupted = original.inner;
        corrupted[0] ^= 0xFF;
        corrupted[1] ^= 0xFF;
        corrupted[2] ^= 0xFF;
        let recovered =
            Hash::validate(corrupted).expect("multi-byte corruption should be recovered");
        assert_eq!(original, recovered);
    }
    ...
    fn validate_below_min_recoverable_fails() {
        let original = Hash::hash(b"below min").expect("hashing should succeed");
    ...
    fn validate_min_recoverable_length_binary() {
        let original = Hash::hash(b"min binary").expect("hashing should succeed");
    ...
    fn validate_full_length_base64() {
        let original = Hash::hash(b"full").expect("hashing should succeed");
        let s = original.to_string();
        assert_eq!(s.len(), HASH_SIZE);
        let validated =
            Hash::validate(&s).expect("validation of a full-length hash string should succeed");
        assert_eq!(original, validated);
    }

    #[test]
    fn validate_full_length_binary() {
        let original = Hash::hash(b"full binary").expect("hashing should succeed");
    ...
    fn validate_accepts_string() {
        let original = Hash::hash(b"string").expect("hashing should succeed");
        let s = original.to_string();
    ...
    fn validate_idempotent() {
        ...
        let v1 = Hash::validate(original.to_string())
            .expect("validation of an uncorrupted hash should succeed");
        let v2 = Hash::validate(v1.to_string()).expect("revalidation should succeed");
        let v3 = Hash::validate(v2.to_string()).expect("revalidation should succeed");
