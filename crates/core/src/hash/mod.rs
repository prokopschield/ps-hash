...
mod tests {
    ...
    fn hash_function_delegates_to_method() {
        ...
        let via_fn = hash(data).expect("hashing should succeed");
        let via_method = Hash::hash(data).expect("hashing should succeed");
    ...
    fn hash_inner_field_has_correct_size() {
        let h = hash(b"test").expect("hashing should succeed");
